...
TIME_PER_ATTENTION_BLOCK = 240 # Seconds, *Note - it can't be less than 22 seconds!!!!
EXPERIMENT_TYPE = 'MAIN' # Can be either 'TEST', 'PRACTICE' or 'MAIN'


#FIXME Ensure script is in source and cwd is the main folder
...
#FIXME PIN function for correct ids
...
def sortSoundConditionTriggerID(sound_condition_PIN_tracker: int, sound_conditions_list: list = SOUND_CONDITIONS) -> int:

    """  
    Used specifically to send the correct pin trigger for each sound condition in Routine "Port"
    to the Biopac EEG. 

    Input: 
    
    sound_condition_pin_tracker - initialized outside of the Block loop and incremented
    +1 every trial

    sound_conditions_list - randomly generated sample of the sound cond sequencing

    Output: 

    integer - that the PsychoPy script should send to the Biopac EEG system via the paralel port.

    """

    # Create a dict from the SOUND CONDITION LIST (It is used for the custom EEG triggers)
    SOUND_CONDITION_DICT = {num_id : condition for num_id, condition in enumerate(sound_conditions_list)}
    
    id_to_sound_cond_mapping = {
        'sounds/silence.wav' : 16,
        'sounds/pink_track.wav' : 17,
        'sounds/lofi_track.wav' : 18
    }

    return id_to_sound_cond_mapping[SOUND_CONDITION_DICT[sound_condition_PIN_tracker]]
...
# Setup the Window
win = visual.Window(
    size=[600, 600], fullscr=True, screen=0, 
...
port_start_experiment = parallel.ParallelPort(address='0x0378')
...
start_experimentComponents = [port_start_experiment]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *port_start_experiment* updates
    if t >= 0.0 and port_start_experiment.status == NOT_STARTED:
        # keep track of start time/frame for later
        port_start_experiment.tStart = t  # not accounting for scr refresh
        port_start_experiment.frameNStart = frameN  # exact frame index
        win.timeOnFlip(port_start_experiment, 'tStartRefresh')  # time at next scr refresh
        port_start_experiment.status = STARTED
        win.callOnFlip(port_start_experiment.setData, int(10))
    frameRemains = 0.0 + 1.0- win.monitorFramePeriod * 0.75  # most of one frame period left
    if port_start_experiment.status == STARTED and t >= frameRemains:
        # keep track of stop time/frame for later
        port_start_experiment.tStop = t  # not accounting for scr refresh
        port_start_experiment.frameNStop = frameN  # exact frame index
        win.timeOnFlip(port_start_experiment, 'tStopRefresh')  # time at next scr refresh
        port_start_experiment.status = FINISHED
        win.callOnFlip(port_start_experiment.setData, int(0))
...
if port_start_experiment.status == STARTED:
    win.callOnFlip(port_start_experiment.setData, int(0))
thisExp.addData('port_start_experiment.started', port_start_experiment.tStart)
thisExp.addData('port_start_experiment.stopped', port_start_experiment.tStop)
...
for thisBlock in block:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and p_port.status == NOT_STARTED:
            ...
            ### Start-Of-Custom-Code ###
            # We use this block of code to get unique

            win.callOnFlip(p_port.setData, int(sortSoundConditionTriggerID(
                sound_condition_PIN_tracker
            )))
            sound_condition_PIN_tracker += 1
...
#            soundCondPort.setData(int(0))
    ...
    frameN = -1
    continueRoutine = True
    routineTimer.add(getBreakSeconds())
    # update component parameters for each repeat
    # keep track of which components have finished

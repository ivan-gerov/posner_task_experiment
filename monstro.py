...
@repeater(1)
def posner_routine(endExpNow=endExpNow):
    # set up handler to look after randomisation of conditions etc

    conditionList = [{'targetX': -0.5, 'cueOri': 0, 'congr': 1, 'corrAns': 'left'}, {'targetX': -0.5, 'cueOri': 0, 'congr': 1, 'corrAns': 'left'}, {'targetX': -0.5, 'cueOri': 0, 'congr': 1, 'corrAns': 'left'}, {'targetX': -0.5, 'cueOri': 180, 'congr': 0, 'corrAns': 'left'},
                     {'targetX': 0.5, 'cueOri': 180, 'congr': 1, 'corrAns': 'right'}, {'targetX': 0.5, 'cueOri': 180, 'congr': 1, 'corrAns': 'right'}, {'targetX': 0.5, 'cueOri': 180, 'congr': 1, 'corrAns': 'right'}, {'targetX': 0.5, 'cueOri': 0, 'congr': 0, 'corrAns': 'right'}]

    trials = data.TrialHandler(nReps=1, method='random',
                               extraInfo=expInfo, originPath=-1,
                               trialList=conditionList,
    ...
    for thisTrial in trials:
        ...
        target.setPos([thisTrial['targetX'], 0])
        key_resp.keys = []
        key_resp.rt = []
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                ...
                if len(theseKeys):
                    ...
                    # was this 'correct'?
                    if (key_resp.keys == str(thisTrial['corrAns'])) or (key_resp.keys == thisTrial['corrAns']):
                        key_resp.corr = 1
                    else:
                        key_resp.corr = 0
...
def sounds_routine():
    ...
    conditionList = [{'sounds': 'sounds/lofi/happy.wav'}, 
                     {'sounds':'sounds/lofi/buddy.wav'}]
    # set up handler to look after randomisation of conditions etc
    trials = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=conditionList,
    ...
    for thisTrial in trials:
        ...
        # update component parameters for each repeat
        sound_1.setSound(thisTrial['sounds'], secs=60, hamming=True)

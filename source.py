...
cue = visual.ImageStim(
    ...
    ori=1.0, pos=(0, 0), size=(0.10, 0.10),
    ...
    for thisPosner_task in posner_task:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if correct_key_resp.status == NOT_STARTED and tThisFlip >= 1.7-frameTolerance:
                ...
                # time at next scr refresh
                win.timeOnFlip(correct_key_resp, 'tStartRefresh')
                correct_key_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                # t=0 on next screen flip
                win.callOnFlip(correct_key_resp.clock.reset)
                # clear events on next screen flip
                win.callOnFlip(correct_key_resp.clearEvents,
                               eventType='keyboard')
            ...
            if early_resp.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                ...
                # time at next scr refresh
                win.timeOnFlip(early_resp, 'tStartRefresh')
                early_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                # t=0 on next screen flip
                win.callOnFlip(early_resp.clock.reset)
                # clear events on next screen flip
                win.callOnFlip(early_resp.clearEvents, eventType='keyboard')
        ...
        posner_task.addData('correct_key_resp.started',
                            correct_key_resp.tStartRefresh)
        posner_task.addData('correct_key_resp.stopped',
                            correct_key_resp.tStopRefresh)

...
sound_conditions = ['sounds/lofi/buddy.wav', 'sounds/lofi/happy.wav']
background_sound= sound.Sound(sound_conditions[randint(0, 2)])
...
trials_2 = data.TrialHandler(nReps=9, method='random', 
...
for thisTrial_2 in trials_2:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if break_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > break_text.tStartRefresh + 60-frameTolerance:
                # keep track of stop time/frame for later
    ...
    for thisComponent in break_2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials_2.addData('break_text.started', break_text.tStartRefresh)
    trials_2.addData('break_text.stopped', break_text.tStopRefresh)
    thisExp.nextEntry()
    
# completed 9 repeats of 'trials_2'
